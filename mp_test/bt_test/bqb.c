...
static int bqb_write2pc(unsigned char* diag_data, int r_cnt, struct device *uart) {
	...
	do {
		...
		if (w_cnt < 0) {
			BT_LOG("bqb_write2pc no data write:%d ,%d\n", w_cnt, errno);
		}
		BT_LOG("bqb_write2pc ,write : %d\n",w_cnt);
		r_cnt -= w_cnt;
		offset += w_cnt;
		BT_LOG("bqb_write2pc, rcnt : %d\n", r_cnt);
	} while (r_cnt > 0);

	BT_LOG("bqb_write2pc ,write finished, return offset : %d\n",offset);
	return offset;
}
...
{
	do {
		BT_LOG("userial_transmit wait for write\n");
		k_sem_take(&rx_sem, K_FOREVER);
		BT_LOG("userial_transmit write len: %d\n", rx_data.len);
...
void bqb_recv_cb(unsigned char *data, int len)
{
	...
	rx_data.len = len;
	BT_LOG("bqb_recv_cb copy data: %d\n", len);
...
int bqb_enable(struct device *uart)
{
	...
	if (recv_serial_buf == NULL) {
		BT_LOG("%s malloc recv_serial_buf failed\n",__func__);
		return -1;
	}
		...
		switch (receive_data->state) {
			...
			case PREAMBLE:
				...
				if (receive_data->bytes_remaining == 0) {
					...
					BT_LOG("%s, recv data size =: 0x%02x\n", __func__, buffer_size);
			...
			case IGNORE:
				...
				break;
			case FINISHED:
				BT_LOG("%s ,the state machine should not have been left in the finished state.\n", __func__);
			break;
			default:
				BT_LOG("%s ,PARSE DEFAULT\n",__func__);
			break;
		}
